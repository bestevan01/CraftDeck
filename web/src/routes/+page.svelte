<script lang="ts">
	...
	// FR-38/39: 2FA setup modal. totpEnabled gates the "외부 접속" toggle
		...
		<div class="flex gap-2">
			...
			{#if isLoggedIn}
				...
				<button
					...
					title={totpEnabled ? '이미 설정되어 있습니다' : '외부 접속을 켜려면 먼저 설정해야 합니다'}
				>
					{totpEnabled ? '2단계 인증 (설정됨)' : '2단계 인증 설정'}
				</button>
				<button
					class="border-border rounded-md border px-4 py-2 text-sm font-medium"
					onclick={logout}
		...
		>
			<div class="mb-3 flex items-center justify-between">
				...
				<button
					type="button"
					class="text-muted-foreground text-sm"
					onclick={() => (showTOTPModal = false)}>&times;</button
				>
			</div>
			...
			{:else if totpBackupCodes}
				<p class="text-sm">설정 완료됐습니다. 아래 백업 코드를 안전한 곳에 저장하세요 -- 다시 볼 수 없습니다.</p>
				...
				<button
					type="button"
					class="bg-primary text-primary-foreground mt-3 w-full rounded-md px-4 py-2 text-sm font-medium"
					onclick={() => (showTOTPModal = false)}
				>
					닫기
				</button>
			...
			{:else}
				...
				<form class="mt-4 space-y-4" onsubmit={submitTOTPVerify}>
					<div>
						<label class="mb-1 block text-sm font-medium" for="totp-verify-code">인증 앱의 6자리 코드</label>
						<input
							id="totp-verify-code"
							type="text"
							inputmode="numeric"
