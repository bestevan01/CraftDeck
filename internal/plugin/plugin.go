...
type Plugin struct {
	...
	VersionNumber         string    `json:"version_number,omitempty"` // Modrinth's own version string, e.g. "1.2.0-beta.3"; empty for uploads
	VersionChannel        string    `json:"version_channel,omitempty"` // "release", "beta", or "alpha"; empty for uploads
...
func (r *Repository) Create(ctx context.Context, p *Plugin) error {
	...
	var versionNumber, versionChannel sql.NullString
	if p.VersionNumber != "" {
		versionNumber = sql.NullString{String: p.VersionNumber, Valid: true}
	}
	if p.VersionChannel != "" {
		versionChannel = sql.NullString{String: p.VersionChannel, Valid: true}
	}
	_, err := r.db.ExecContext(ctx, `
		INSERT INTO plugins (
			id, instance_id, source, modrinth_project_id, modrinth_version_id,
			filename, title, version_number, version_channel, sha512, enabled,
			installed_as_dependency, parent_plugin_id, created_at
		) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
		p.ID, p.InstanceID, p.Source, p.ModrinthProjectID, p.ModrinthVersionID,
		p.Filename, title, versionNumber, versionChannel, p.SHA512, p.Enabled,
		p.InstalledAsDependency, parentPluginID,
...
func scanPlugin(row rowScanner) (*Plugin, error) {
	...
	var modrinthProjectID, modrinthVersionID, title, versionNumber, versionChannel, sha512, parentPluginID sql.NullString
	err := row.Scan(
		&p.ID, &p.InstanceID, &p.Source, &modrinthProjectID, &modrinthVersionID,
		&p.Filename, &title, &versionNumber, &versionChannel, &sha512, &p.Enabled,
		&p.InstalledAsDependency, &parentPluginID, &createdAt,
	)
	if err != nil {
		return nil, err
	}
	...
	p.VersionNumber = versionNumber.String
	p.VersionChannel = versionChannel.String
